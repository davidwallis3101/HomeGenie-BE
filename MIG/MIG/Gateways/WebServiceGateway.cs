// <copyright file="WebServiceGateway.cs" company="Bounz">
    ...
    public class WebServiceGateway : IMigGateway, IDisposable
    {
        ...
        private void Worker(object state)
        {
            ...
            try
            {
                ...
                string logExtras = string.Empty;

                if (servicePassword == string.Empty || requestHasAuthorizationHeader)
                {
                    bool verified = false;
                    ...
                    if (servicePassword == string.Empty || authUser == serviceUsername && Utility.Encryption.SHA1.GenerateHashString(authPass) == servicePassword)
                    {
                        verified = true;
                    }
                    ...
                    if (verified)
                    {
                        ...
                        if (url == string.Empty || url.TrimEnd('/') == baseUrl.TrimEnd('/'))
                        {
                        ...
                        else
                        {
                            ...
                            MigService.Log.Info(new MigEvent(this.GetName(), remoteAddress, "HTTP", request.HttpMethod, $"{response.StatusCode} {request.RawUrl} [OPEN]"));
                ...
                MigService.Log.Info(new MigEvent(this.GetName(), remoteAddress, "HTTP", request.HttpMethod, $"{response.StatusCode} {request.RawUrl}{logExtras}"));
            }
            catch (Exception ex)
            {
                MigService.Log.Error(ex);
        ...
        private void HandleEventsRoute(HttpListenerRequest request, HttpListenerResponse response, HttpListenerContext context, string remoteAddress)
        {
            // Server sent events
                ...
                do
                {
                    ...
                    if (bufferedData.Count > 0)
                    {
                        foreach (SseEvent entry in bufferedData)
                        {
                            ...
                            {
                                MigService.Log.Info(new MigEvent(this.GetName(), remoteAddress, "HTTP", request.HttpMethod, $"{response.StatusCode} {request.RawUrl} [ERROR: {e.Message}]"));
                                connected = false;
                                break;
                            }
                        }
        ...
        private void StartListener(IEnumerable<string> prefixes)
        {
            ...
            if (servicePassword != string.Empty)
            {
        ...
        private string GetWebFilePath(string file)
        {
            ...
            file = file.Replace(baseUrl, string.Empty);
