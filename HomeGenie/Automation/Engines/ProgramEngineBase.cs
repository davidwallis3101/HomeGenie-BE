// <copyright file="ProgramEngineBase.cs" company="Bounz">
            ...
            _programThread = new Thread(() =>
            {
                try
                {
                            ...
                            .Equals(typeof(System.Reflection.TargetException)))
                    {
                        ...
                        Homegenie.ProgramManager.RaiseProgramModuleEvent(ProgramBlock, Properties.RuntimeError, PrepareExceptionMessage(CodeBlockEnum.CR, result.Exception));
            ...
            lock (ProgramBlock.OperationLock)
            {
                try
                {
                    ...
                    if (result != null && result.Exception != null)
                    {
                        ...
                        ProgramBlock.ScriptErrors = JsonConvert.SerializeObject(error);
                        _log.Error(result.Exception, "Error while evaluating condition in program {0}", ProgramBlock.Address);
                        Homegenie.ProgramManager.RaiseProgramModuleEvent(ProgramBlock, Properties.RuntimeError, PrepareExceptionMessage(CodeBlockEnum.TC, result.Exception));
                ...
                catch (Exception ex)
                {
                        ...
                        !ex.GetType().Equals(typeof(ThreadAbortException)))
                    {
                        ...
                        ProgramBlock.ScriptErrors = JsonConvert.SerializeObject(error);
                        Homegenie.ProgramManager.RaiseProgramModuleEvent(ProgramBlock, Properties.RuntimeError, PrepareExceptionMessage(CodeBlockEnum.TC, ex));
                        TryToAutoRestart();
                    }
                }
            }
